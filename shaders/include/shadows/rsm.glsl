    ...
    vec3 compute_rsm_gi(vec3 fragment_normal_world) {
        ...
        distort_shadow_clip_position(fragment_position_shadow_clip.xyz);
        ...
        for (uint idx = 0; idx < RSM_SAMPLES; idx += 1) {
            vec2 offset = sqrt(radius) * compute_vogel_disk_sample_uv(idx, RSM_SAMPLES);
            ...
            distort_shadow_clip_position(sample_position_shadow_clip.xyz);
