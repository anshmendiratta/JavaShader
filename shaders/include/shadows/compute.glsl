...
vec3 _get_soft_shadow(in vec4 shadow_clip_position, vec3 normal_world, float lightmap_sky) {
    ...
    // _xonk_gri_emin_shadow_fix(frag_world_position, normal_world, lightmap_sky);
    // shadow_clip_position = shadow_view_to_shadow_clip(feet_to_shadow_view(world_to_feet(frag_world_position)));
