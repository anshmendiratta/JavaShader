    ...
    void _xonk_gri_emin_shadow_fix(vec3 frag_world_position, vec3 frag_world_normal, float lightmap_sky); // not necessary with eldeston's normal offset
    ...
    vec3 _get_soft_shadow(vec4 shadow_clip_position, vec3 normal_world, float lightmap_sky) {
        ...
        for (int idx = 0; idx < SHADOW_BLUR_SAMPLE_COUNT; idx += 1) {
            ...
            shadow_accumulator += _get_shadow(sample_uv_screen_position);
        }
    ...
    void _xonk_gri_emin_shadow_fix(vec3 frag_world_position, vec3 frag_world_normal, float lightmap_sky) {
