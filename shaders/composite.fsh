...
// FIX: Bug with deferred lighting. Compare with tutorial image.
...
void main() {
    ...
    // vec3 sunlight = clamp(normal_aligned_with_light_vector, 0.0, 1.0) * lightmap.g * sunlight_color; // Multiply by the skylight from the light map since if an object is hidden from the sky, the object is also hidden from the sun.
